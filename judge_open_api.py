...
class OpenAiJudge:
    ...
    '''
    Tạo message từ template, thay thế biến bằng kwargs
    Kết quả: Điểm số đánh giá (float hoặc None nếu từ chối)
    '''
    ...
    async def logprob_probs(self, messages) -> dict:
        ...
        completion = await openai.chat.completions.create(
            ...
            messages=messages,
            max_tokens=1,
            temperature=0, # Không random
            logprobs=True, # Lấy log probability
            top_logprobs=20, # 20 token có xác suất cao nhất
            seed=0
        )
        try:
        ...
        '''
        Chuyển log probability → probability bằng e^(logprob)
        Kết quả: Dictionary {token: xác_suất}
        '''
        result = {}
        for el in logprobs:
