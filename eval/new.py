...
class Question():
    def __init__(self, id, paraphrases, judge_prompts, temperature=1, system=None, judge="gpt-4o", judge_eval_type="0_100", **ignored_extra_args):
        self.id = id
    ...
    async def eval(self, model, tokenizer, coef, vector=None, layer=None, max_tokens=1000, n_per_question=100, steering_type="response", lora_path=None):
        paraphrases, conversations = self.get_input(n_per_question)
        prompts, answers = sample(model, tokenizer, conversations, coef, vector, layer, temperature=self.temperature, max_tokens=max_tokens, steering_type=steering_type, lora_path=lora_path)
        df = pd.DataFrame([
    ...
    @staticmethod
    async def eval_batched(questions, model, tokenizer, coef, vector=None, layer=None, n_per_question=100, max_concurrent_judges=100, max_tokens=1000, steering_type="response", lora_path=None):
        ...
        prompts, answers = sample(model, tokenizer, all_conversations, coef, vector, layer, temperature=questions[0].temperature, max_tokens=max_tokens, steering_type=steering_type, lora_path=lora_path)
        ...
        for q_idx, (q, df, pointers) in enumerate(zip(questions, question_dfs, sample_pointers)):
            for metric, judge in q.judges.items():
                for row_idx, global_idx in enumerate(pointers):
                    all_tasks.append((judge, all_paraphrases[global_idx], answers[global_idx]))
        ...
        tasks = [run_with_semaphore(i, j, q, a) for i, (j, q, a) in enumerate(all_tasks)]
...
def main(model, trait, output_path, coef=0, vector_path=None, layer=None, steering_type="response", max_tokens=1000, n_per_question=10, batch_process=True, max_concurrent_judges=100, persona_instruction_type=None, assistant_name=None, judge_model="gpt-4.1-mini-2025-04-14", version="extract", overwrite=False):
    ...
    if coef != 0:
        ...
        llm, tokenizer, lora_path = load_model_basic(model)
        vector = None
    ...
    if batch_process:
        ...
        outputs_list = asyncio.run(Question.eval_batched(questions, llm, tokenizer, coef, vector, layer, n_per_question, max_concurrent_judges, max_tokens, steering_type=steering_type, lora_path=lora_path))
